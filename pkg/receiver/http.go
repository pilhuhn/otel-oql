...
func (r *HTTPReceiver) handleTraces(w http.ResponseWriter, req *http.Request) {
	...
	if r.debugIngestion {
		fmt.Printf("[DEBUG INGESTION] HTTP: Received traces request from %s\n", req.RemoteAddr)
	}
	...
	if err != nil {
		if r.debugIngestion {
			fmt.Printf("[DEBUG INGESTION] HTTP: Failed to read body: %v\n", err)
		}
	...
	if r.debugIngestion {
		fmt.Printf("[DEBUG INGESTION] HTTP: Read %d bytes from request body\n", len(body))
	}
	...
	if err := otlpReq.UnmarshalProto(body); err != nil {
		if r.debugIngestion {
			fmt.Printf("[DEBUG INGESTION] HTTP: Failed to unmarshal: %v\n", err)
		}
	...
	if r.debugIngestion {
		fmt.Printf("[DEBUG INGESTION] HTTP: Successfully unmarshaled traces\n")
	}
	...
	if !ok {
		if r.debugIngestion {
			fmt.Printf("[DEBUG INGESTION] HTTP: Tenant ID not found in context\n")
		}
	...
	if r.debugIngestion {
		fmt.Printf("[DEBUG INGESTION] HTTP: Tenant ID: %d\n", tenantID)
	}

	// Ingest traces
	if err := r.ingester.IngestTraces(ctx, tenantID, otlpReq.Traces()); err != nil {
		if r.debugIngestion {
			fmt.Printf("[DEBUG INGESTION] HTTP: Failed to ingest traces: %v\n", err)
		}
	...
	if r.debugIngestion {
		fmt.Printf("[DEBUG INGESTION] HTTP: Successfully ingested traces\n")
	}
...
func (r *HTTPReceiver) handleMetrics(w http.ResponseWriter, req *http.Request) {
	...
	if r.debugIngestion {
		fmt.Printf("[DEBUG INGESTION] HTTP: Received metrics request from %s\n", req.RemoteAddr)
	}
	...
	if r.debugIngestion {
		fmt.Printf("[DEBUG INGESTION] HTTP: Successfully unmarshaled metrics\n")
	}
	...
	if err := r.ingester.IngestMetrics(ctx, tenantID, otlpReq.Metrics()); err != nil {
		if r.debugIngestion {
			fmt.Printf("[DEBUG INGESTION] HTTP: Failed to ingest metrics: %v\n", err)
		}
		r.obs.RecordError(ctx, "ingest_failure", "http_receiver")
	...
	if r.debugIngestion {
		fmt.Printf("[DEBUG INGESTION] HTTP: Successfully ingested metrics\n")
	}
...
func (r *HTTPReceiver) handleLogs(w http.ResponseWriter, req *http.Request) {
	...
	if r.debugIngestion {
		fmt.Printf("[DEBUG INGESTION] HTTP: Received logs request from %s\n", req.RemoteAddr)
	}
	...
	if r.debugIngestion {
		fmt.Printf("[DEBUG INGESTION] HTTP: Successfully unmarshaled logs\n")
	}
	...
	if err := r.ingester.IngestLogs(ctx, tenantID, otlpReq.Logs()); err != nil {
		if r.debugIngestion {
			fmt.Printf("[DEBUG INGESTION] HTTP: Failed to ingest logs: %v\n", err)
		}
		r.obs.RecordError(ctx, "ingest_failure", "http_receiver")
	...
	if r.debugIngestion {
		fmt.Printf("[DEBUG INGESTION] HTTP: Successfully ingested logs\n")
	}
